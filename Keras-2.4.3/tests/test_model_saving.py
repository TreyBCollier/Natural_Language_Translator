...
def test_sequential_model_saving():
    ...
    model.compile(loss=losses.MeanSquaredError(),        optimizer=optimizers.RMSprop(lr=0.0001),        metrics=[metrics.categorical_accuracy],        sample_weight_mode='temporal')
...
def test_sequential_model_saving_2():
    ...
    load_kwargs = {'custom_objects': {'custom_opt': custom_opt,      'custom_loss': custom_loss}}
    _, fname = tempfile.mkstemp('.h5')
    save_model(model, fname)
...
def _get_sample_model_and_input():
    ...
    model.compile(loss=losses.MSE,        optimizer=optimizers.Adam(),        metrics=[metrics.categorical_accuracy])
    x = np.random.random((1, 3))
    y = np.random.random((1, 3))
    model.train_on_batch(x, y)
...
def DISABLED_test_model_saving_to_binary_stream():
    ...
    with temp_filename('h5') as fname:save directly to binary file
        with open(fname, 'wb') as raw_file:  save_model(model, raw_file)Load the data the usual way, and make sure the model is intact.
        with h5py.File(fname, mode='r') as h5file:  loaded_model = load_model(h5file)
    out2 = loaded_model.predict(x)
    assert_allclose(out, out2, atol=1e-05)
...
def DISABLED_test_model_loading_from_binary_stream():
    ...
    with temp_filename('h5') as fname:save the model the usual way
        with h5py.File(fname, mode='w') as h5file:  save_model(model, h5file)Load the data binary, and make sure the model is intact.
        with open(fname, 'rb') as raw_file:  loaded_model = load_model(raw_file)
    out2 = loaded_model.predict(x)
    assert_allclose(out, out2, atol=1e-05)
...
def test_loading_weights_by_name_2():
    """
    test loading model weights by name on:both sequential and functional api modelsdifferent architecture with shared names
    """
    custom_loss = losses.mse
...
def test_loading_weights_by_name_skip_mismatch():
    """
    test skipping layers while loading model weights by name on:sequential model
    """
...
def test_saving_lambda_custom_objects():
    ...
    model = Model(inputs, outputs)
    model.compile(loss=losses.MSE,        optimizer=optimizers.RMSprop(lr=0.0001),        metrics=[metrics.categorical_accuracy])
    x = np.random.random((1, 3))
    y = np.random.random((1, 3))
    model.train_on_batch(x, y)
...
def test_saving_lambda_numpy_array_arguments():
    ...
    outputs = Lambda(lambda image, mu, std: (image - mu) / std,arguments={'mu': mean, 'std': std})(inputs)
    model = Model(inputs, outputs)
    model.compile(loss='mse', optimizer='sgd', metrics=['acc'])
...
def test_saving_model_with_long_layer_names():
    ...
    with h5py.File(fname, 'r') as h5file:
        n_layer_names_arrays = len([attr for attr in h5file['model_weights'].attrs    if attr.startswith('layer_names')])
...
def test_saving_model_with_long_weights_names():
    ...
    # of weight names.
    with h5py.File(fname, 'r') as h5file:
        attrs = [attr for attr in h5file['model_weights']['nested_model'].attrs       if attr.startswith('weight_names')]
...
def _make_nested_model(input_shape, layer, level=1, model_type='func'):
    ...
    def make_nested_seq_model(input_shape, layer, level=1):
        model = layer
        for i in range(1, level + 1):  layers = [InputLayer(input_shape), model] if (i == 1) else [model]  model = Sequential(layers)
        return model
    ...
    def make_nested_func_model(input_shape, layer, level=1):
        ...
        model = layer
        for i in range(level):  model = Model(input, model(input))
        return model
...
def test_model_saving_with_rnn_initial_state_and_args():
    class CustomRNN(LSTM):
        def call(self, inputs, arg=1, mask=None, training=None, initial_state=None):  if isinstance(inputs, list):      inputs = inputs[:]      shape = K.int_shape(inputs[0])      inputs[0] *= arg      inputs[0]._keras_shape = shape  # for theano backend  else:      shape = K.int_shape(inputs)      inputs *= arg      inputs._keras_shape = shape  # for theano backend  return super(CustomRNN, self).call(inputs, mask, training, initial_state)

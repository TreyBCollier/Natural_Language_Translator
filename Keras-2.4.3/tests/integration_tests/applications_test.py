...
def _get_output_shape(model_fn):
    if K.backend() == 'cntk':Create model in a subprocess so thatthe memory consumed by InceptionResNetV2 will bereleased back to the system after this test(to deal with OOM error on CNTK backend).TODO: remove the use of multiprocessing from these testsonce a memory clearing mechanismis implemented in the CNTK backend.
        def target(queue):  model = model_fn()  queue.put(model.output_shape)
        queue = Queue()
        p = Process(target=target, args=(queue,))
        p.start()
        p.join()The error in a subprocess won't propagateto the main process, so we check if the modelis successfully created by checking if the output shapehas been put into the queue

...
def test_vector_classification():
    ...
    '''
    (x_train, y_train), (x_test, y_test) = get_test_data(num_train=500,   num_test=200,   input_shape=(20,),   classification=True,   num_classes=num_classes)
    ...
    model.summary()
    history = model.fit(x_train, y_train, epochs=15, batch_size=16,   validation_data=(x_test, y_test),   verbose=0)
...
def test_vector_classification_functional():
    ...
    model.compile(loss=keras.losses.sparse_categorical_crossentropy,        optimizer=keras.optimizers.Adam(1e-3),        metrics=['accuracy'])
    history = model.fit(x_train, y_train, epochs=15, batch_size=16,   validation_data=(x_train, y_train),   verbose=0)
    assert(history.history['val_accuracy'][-1] > 0.8)
...
def test_vector_regression():
    ...
    '''
    (x_train, y_train), (x_test, y_test) = get_test_data(num_train=500,   num_test=200,   input_shape=(20,),   output_shape=(num_classes,),   classification=False)

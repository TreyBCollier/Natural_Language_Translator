...
def test_merge_broadcast():
    ...
    if K.backend() == 'tensorflow':
        ...
        i2 = layers.Input(shape=(None,))
        ops = [layers.add, layers.maximum]
        for op in ops:  o = op([i1, i2])  assert K.int_shape(o) == (None, None, None)  model = models.Model([i1, i2], o)
  x1 = np.random.random((2, 4, 5))  x2 = np.random.random((2, 5))  out = model.predict([x1, x2])  assert out.shape == (2, 4, 5)
        K.ndim = k_ndim

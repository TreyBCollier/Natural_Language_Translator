...
def test_serialize():
    ...
    class MockMetric:
        def __init__(self):  self.__name__ = "mock_metric"

    mock = MockMetric()
    found = metrics.serialize(mock)
    assert found == "mock_metric"

    found = metrics.deserialize('mock_metric',custom_objects={'mock_metric': True})
...
@pytest.mark.skipif((K.backend() == 'cntk'),'CNTK backend does not support top_k yet')

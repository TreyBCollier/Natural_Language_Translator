...
def test_node_construction():
    ...
    # with pytest.raises(AttributeError):dense.input
    # with pytest.raises(AttributeError):dense.output
    # with pytest.raises(AttributeError):dense.input_mask
    # with pytest.raises(AttributeError):dense.output_mask
...
def test_recursion():
    ...
    fn_outputs = fn([np.random.random((10, 32)), np.random.random((10, 32)),np.random.random((10, 32)), np.random.random((10, 32))])
...
def test_load_layers():
    ...
    for j in range(2):  # bidirectional
        for i in range(4):  weights_bi_conv_old.append(np.zeros((3, 3, 15, 10)))  # kernel  weights_bi_conv_old.append(np.zeros((3, 3, 10, 10)))  # recurrent kernel  weights_bi_conv_old.append(np.zeros((10,)))  # bias
...
@pytest.mark.parametrize("layer", [
    ...
    layers.ConvLSTM2D(5, (3, 3), input_shape=[6, 6, 6, 6], data_format='channels_first'),
...
@pytest.mark.parametrize("layer", [
    ...
    layers.Conv2DTranspose(2, (5, 5),      input_shape=[7, 7, 3],      data_format='channels_first'),
...
def test_activity_regularization_with_model_composition():
    ...
    net_a_input = Input((2,))
    net_a = net_a_input
    net_a = Dense(2, kernel_initializer='ones',        use_bias=False,        activity_regularizer=reg)(net_a)
...
def test_multi_output_mask():
    ...
    class TestMultiOutputLayer(Layer):
        def __init__(self, **kwargs):  super(TestMultiOutputLayer, self).__init__(**kwargs)

        def call(self, inputs, **kwargs):  return [K.abs(inputs), K.abs(inputs)]

        def compute_output_shape(self, input_shape):  out_shape = super(TestMultiOutputLayer, self).compute_output_shape(      input_shape)  return [out_shape, out_shape]

    class TestMultiInputLayer(Layer):
        def __init__(self, **kwargs):  super(TestMultiInputLayer, self).__init__(**kwargs)

        def call(self, inputs, **kwargs):  negative, positive = inputs  return negative + positive
...
def test_constant_initializer_with_numpy():
    model = Sequential()
    model.add(Dense(2, input_shape=(3,)kernel_initializer=Constant(1.)))
    model.add(Dense(3))
...
@pytest.mark.skipif(K.backend() == 'cntk','Float64 not supported with CNTK.')
def test_initialization_dtype():
    class TestLayer(Layer):
        def __init__(self):  super(TestLayer, self).__init__(dtype='int64')  self.w = self.add_weight('w', [], initializer=Constant(1))

    layer = TestLayer()
    assert K.dtype(layer.w) == 'int64'

    class TestModel(Model):
        def __init__(self):  super(TestModel, self).__init__(dtype='int64')  self.w = self.add_weight('w', [], initializer=Constant(1))

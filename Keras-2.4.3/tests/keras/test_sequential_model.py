...
def test_sequential(in_tmpdir):
    ...
    def data_generator(x, y, batch_size=50):
        ...
        while 1:  batches = make_batches(len(x_test), batch_size)  for batch_index, (batch_start, batch_end) in enumerate(batches):      batch_ids = index_array[batch_start:batch_end]      x_batch = x[batch_ids]      y_batch = y[batch_ids]      yield (x_batch, y_batch)
...
def test_clone_functional_model():
    ...
    if K.backend() == 'tensorflow':Everything should work in a new session.
        K.clear_session()
...
def test_clone_functional_model_with_multi_outputs():
    ...
    class SwapLayer(keras.layers.Layer):
        def call(self, inputs, **kwargs):  return [inputs[1], inputs[0]]

        def compute_output_shape(self, input_shape):  return [input_shape[1], input_shape[0]]

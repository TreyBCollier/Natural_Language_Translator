...
class TestMetricsCorrectnessMultiIO(object):
    ...
    def _get_compiled_multi_io_model(self):
        ...
        model.compile(  optimizer='rmsprop',  loss=losses.MeanSquaredError(),  metrics=[metrics.MeanSquaredError(name='mean_squared_error')],  weighted_metrics=[      metrics.MeanSquaredError(name='mean_squared_error_2')  ])
        return model
...
Total loss with weights = 32.5 + 35 = 67.5Total loss without weights = 7.5 + 30 = 37.5
        ...
        self.expected_fit_result_with_weights      'output_1_mean_squared_error': [7.5, 7.5],  'output_2_mean_squared_error': [30, 30],  'output_1_mean_squared_error_2': [9.286, 9.286],  'output_2_mean_squared_error_2': [17.5, 17.5],  'loss': [67.5, 67.5],  'output_1_loss': [32.5, 32.5],  'output_2_loss': [35, 35],
        self.expected_fit_result_with_weights_output_2      'output_1_mean_squared_error': [7.5, 7.5],  'output_2_mean_squared_error': [30, 30],  'output_1_mean_squared_error_2': [7.5, 7.5],  'output_2_mean_squared_error_2': [17.5, 17.5],  'loss': [42.5, 42.5],  'output_1_loss': [7.5, 7.5],  'output_2_loss': [35, 35],
        self.expected_fit_result      'output_1_mean_squared_error': [7.5, 7.5],  'output_2_mean_squared_error': [30, 30],  'output_1_mean_squared_error_2': [7.5, 7.5],  'output_2_mean_squared_error_2': [30, 30],  'loss': [37.5, 37.5],  'output_1_loss': [7.5, 7.5],  'output_2_loss': [30, 30],In the order: 'loss', 'output_1_loss', 'output_2_loss','output_1_mean_squared_error', 'output_1_mean_squared_error_2','output_2_mean_squared_error', 'output_2_mean_squared_error_2'
        self.expected_batch_result_with_weights      67.5, 32.5, 35, 7.5, 9.286, 30, 17.5        self.expected_batch_result_with_weights_output_2      42.5, 7.5, 35, 7.5, 7.5, 30, 17.5        self.expected_batch_result = [37.5, 7.5, 30, 7.5, 7.5, 30, 30]

    def test_fit(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit([self.x, self.x], [self.y1, self.y2],       batch_size=2,       epochs=2,       shuffle=False)
        for key, value in self.expected_fit_result.items():  np.allclose(history.history[key], value, 1e-3)

    def test_fit_with_sample_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit([self.x, self.x], [self.y1, self.y2],       sample_weight={'output_1': self.sample_weight_1,'output_2': self.sample_weight_2},       batch_size=2,       epochs=2,       shuffle=False)
        for key, value in self.expected_fit_result_with_weights.items():  np.allclose(history.history[key], value, 1e-3)
Set weights for one output (use batch size).
        history = model.fit([self.x, self.x], [self.y1, self.y2],       sample_weight={'output_2': self.sample_weight_2},       batch_size=2,       epochs=2,       shuffle=False)

        for key, value in self.expected_fit_result_with_weights_output_2.items():  np.allclose(history.history[key], value, 1e-3)

    def DISABLED_test_fit_with_class_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit([self.x, self.x], [self.y1, self.y2],       class_weight={'output_1': self.class_weight_1,'output_2': self.class_weight_2},       batch_size=2,       epochs=2,       shuffle=False)
        for key, value in self.expected_fit_result_with_weights.items():  np.allclose(history.history[key], value, 1e-3)
Set weights for one output.
        history = model.fit([self.x, self.x], [self.y1, self.y2],       class_weight={'output_2': self.class_weight_2},       batch_size=2,       epochs=2,       shuffle=False)
...
Verify that metric value is same with arbitrary weights and batch size.
    ...
    def test_train_on_batch_with_sample_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        result = model.train_on_batch([self.x, self.x], [self.y1, self.y2],      sample_weight={output_1': self.sample_weight_1,output_2': self.sample_weight_2})
        np.allclose(result, self.expected_batch_result_with_weights, 1e-3)
Set weights for one output.
        result = model.train_on_batch([self.x, self.x], [self.y1, self.y2],      sample_weight={output_2': self.sample_weight_2})
    ...
    def DISABLED_test_train_on_batch_with_class_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        result = model.train_on_batch([self.x, self.x], [self.y1, self.y2],      class_weight={output_1': self.class_weight_1,output_2': self.class_weight_2})
        np.allclose(result, self.expected_batch_result_with_weights, 1e-3)
Set weights for one output.
        result = model.train_on_batch([self.x, self.x], [self.y1, self.y2],      class_weight={output_2': self.class_weight_2})
...
Set weights for one output.
    ...
    def test_fit_generator(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit_generator(  custom_generator_multi_io(), steps_per_epoch=2, epochs=2)
        for key, value in self.expected_fit_result.items():  np.allclose(history.history[key], value, 1e-3)

    def DISABLED_test_fit_generator_with_sample_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit_generator(  custom_generator_multi_io(      sample_weights=[self.sample_weight_1, self.sample_weight_2]),  steps_per_epoch=2,  epochs=2)
        for key, value in self.expected_fit_result_with_weights.items():  np.allclose(history.history[key], value, 1e-3)
Set weights for one output.
        history = model.fit_generator(  custom_generator_multi_io(sample_weights=[None, self.sample_weight_2]),  steps_per_epoch=2,  epochs=2)
        for key, value in self.expected_fit_result_with_weights_output_2.items():  np.allclose(history.history[key], value, 1e-3)

    def DISABLED_test_fit_generator_with_class_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        history = model.fit_generator(  custom_generator_multi_io(),  class_weight={      'output_1': self.class_weight_1,      'output_2': self.class_weight_2,  },  steps_per_epoch=2,  epochs=2)
        for key, value in self.expected_fit_result_with_weights.items():  np.allclose(history.history[key], value, 1e-3)
Set weights for one output.
        history = model.fit_generator(  custom_generator_multi_io(),  class_weight={'output_2': self.class_weight_2},  steps_per_epoch=2,  epochs=2)
        for key, value in self.expected_fit_result_with_weights_output_2.items():  np.allclose(history.history[key], value, 1e-3)
    ...
    def DISABLED_test_eval_generator_with_sample_weight(self):
        self.setUp()
        model = self._get_compiled_multi_io_model()
        eval_result = model.evaluate_generator(  custom_generator_multi_io(      sample_weights=[self.sample_weight_1, self.sample_weight_2]),  steps=2)
        np.allclose(eval_result, self.expected_batch_result_with_weights, 1e-3)
Set weights for one output.
        eval_result = model.evaluate_generator(  custom_generator_multi_io(sample_weights=[None, self.sample_weight_2]),  steps=2)
        np.allclose(eval_result,expected_batch_result_with_weights_output_2, 1e-3)

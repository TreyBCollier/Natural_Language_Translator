...
class MSE_MAE_loss(losses.Loss):
    ...
    def __call__(self, y_true, y_pred, sample_weight=None):
        return (self.mse_fraction * losses.mse(y_true, y_pred  self.mse_fraction) * losses.mae(y_true, y_pred))
...
class TestLossFunctions(object):
    ...
    def test_serializing_loss_class(self):
        ...
        with custom_object_scope({'MSE_MAE_loss': MSE_MAE_loss}):  serialized = losses.serialize(orig_loss_class)

        with custom_object_scope({'MSE_MAE_loss': MSE_MAE_loss}):  deserialized = losses.deserialize(serialized)
    ...
    def test_serializing_model_with_loss_class(self, tmpdir):
        ...
        with custom_object_scope({'MSE_MAE_loss': MSE_MAE_loss}):  loss = MSE_MAE_loss(0.3)  inputs = keras.layers.Input((2,))  outputs = keras.layers.Dense(1, name='model_output')(inputs)  model = keras.models.Model(inputs, outputs)  model.compile(optimizer='sgd', loss={'model_output': loss})  model.fit(np.random.rand(256, 2), np.random.rand(256, 1))  model.save(model_filename)

        with custom_object_scope({'MSE_MAE_loss': MSE_MAE_loss}):  loaded_model = keras.models.load_model(model_filename)  loaded_model.predict(np.random.rand(128, 2))
...
class TestMeanSquaredError:
    ...
    def test_config(self):
        mse_obj = losses.MeanSquaredError(  reduction=Reduction.SUM, name='mse_1')
    ...
    def test_invalid_sample_weight(self):
        ...
        with pytest.raises(Exception):  mse_obj(y_true, y_pred, sample_weight=sample_weight)

    def test_no_reduction(self):
        mse_obj = losses.MeanSquaredError(  reduction=Reduction.NONE)
        y_true = K.constant([1, 9, 2, -5, -2, 6], shape=(2, 3))
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3))
        loss = mse_obj(y_true, y_pred, sample_weight=2.3)
    ...
    def test_sum_reduction(self):
        mse_obj = losses.MeanSquaredError(  reduction=Reduction.SUM)
        y_true = K.constant([1, 9, 2, -5, -2, 6], shape=(2, 3))
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3))
        loss = mse_obj(y_true, y_pred, sample_weight=2.3)
...
class TestMeanAbsoluteError(object):
    ...
    def test_config(self):
        mae_obj = losses.MeanAbsoluteError(  reduction=Reduction.SUM, name='mae_1')
    ...
    def test_invalid_sample_weight(self):
        ...
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3, 1))
        sample_weight = K.constant([3, 6, 5, 0], shape=(2, 2))
        with pytest.raises(Exception):  mae_obj(y_true, y_pred, sample_weight=sample_weight)

    def test_no_reduction(self):
        mae_obj = losses.MeanAbsoluteError(  reduction=Reduction.NONE)
        y_true = K.constant([1, 9, 2, -5, -2, 6], shape=(2, 3))
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3))
        loss = mae_obj(y_true, y_pred, sample_weight=2.3)
    ...
    def test_sum_reduction(self):
        mae_obj = losses.MeanAbsoluteError(  reduction=Reduction.SUM)
        y_true = K.constant([1, 9, 2, -5, -2, 6], shape=(2, 3))
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3))
        loss = mae_obj(y_true, y_pred, sample_weight=2.3)
...
class TestMeanAbsolutePercentageError(object):
    ...
    def test_config(self):
        mape_obj = losses.MeanAbsolutePercentageError(  reduction=Reduction.SUM, name='mape_1')
    ...
    def test_no_reduction(self):
        mape_obj = losses.MeanAbsolutePercentageError(  reduction=Reduction.NONE)
        y_true = K.constant([1, 9, 2, -5, -2, 6], shape=(2, 3))
        y_pred = K.constant([4, 8, 12, 8, 1, 3], shape=(2, 3))
        loss = mape_obj(y_true, y_pred, sample_weight=2.3)
...
class TestMeanSquaredLogarithmicError(object):
    ...
    def test_config(self):
        msle_obj = losses.MeanSquaredLogarithmicError(  reduction=Reduction .SUM, name='mape_1')
...
class TestBinaryCrossentropy(object):
    ...
    def test_config(self):
        bce_obj = losses.BinaryCrossentropy(  reduction=Reduction.SUM, name='bce_1')
...
Loss = max(x, 0) - x * z + log(1 + exp(-abs(x)))(where x = logits and z = y_true)Loss = [(0 + 0 + 0) / 3, 200 / 3]Weighted loss = [0 * 2.3, 66.666 * 2.3]Reduced loss = (0 + 66.666 * 2.3) / 2
...
Loss = max(x, 0) - x * z + log(1 + exp(-abs(x)))(where x = logits and z = y_true)Loss = [(0 + 0 + 0)/3, 200 / 3]Weighted loss = [0 * 4, 66.666 * 3]Reduced loss = (0 + 66.666 * 3) / 2
    ...
    def test_no_reduction(self):
        y_true = K.constant([[1, 0, 1], [0, 1, 1]])
        logits = K.constant([[100.0, -100.0, 100.0], [100.0, 100.0, -100.0]])
        bce_obj = losses.BinaryCrossentropy(  from_logits=True, reduction=Reduction.NONE)
        loss = bce_obj(y_true, logits)
Loss = max(x, 0) - x * z + log(1 + exp(-abs(x)))(where x = logits and z = y_true)Loss = [(0 + 0 + 0)/3, (200)/3]
...
class TestCategoricalCrossentropy(object):
    ...
    def test_config(self):
        cce_obj = losses.CategoricalCrossentropy(  reduction=Reduction.SUM, name='bce_1')
...
Test with logits.
    ...
    def test_no_reduction(self):
        y_true = K.constant([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        logits = K.constant([[8., 1., 1.], [0., 9., 1.], [2., 3., 5.]])
        cce_obj = losses.CategoricalCrossentropy(  from_logits=True, reduction=Reduction.NONE)
        loss = cce_obj(y_true, logits)
    ...
    def test_label_smoothing(self):
        ...
        label_smoothing = 0.1Softmax Cross Entropy Loss: -\sum_i p_i \log q_iwhere for a softmax activation\log q_i = x_i - \log \sum_j \exp x_jx_i - x_max - \log \sum_j \exp (x_j - x_max)For our activations, [100, -100, -100]\log ( exp(0) + exp(-200) + exp(-200) ) = 0so our log softmaxes become: [0, -200, -200]Label smoothing: z' = z * (1 - L) + L/n      1  = 1 - L + L/n      0  = L/nApplying the above two fns to the given input:-0 * (1 - L + L/n) + 200 * L/n + 200 * L/n = 400 L/n
        cce_obj = losses.CategoricalCrossentropy(  from_logits=True, label_smoothing=label_smoothing)
        loss = cce_obj(y_true, logits)
...
class TestSparseCategoricalCrossentropy(object):
    ...
    def test_config(self):
        cce_obj = losses.SparseCategoricalCrossentropy(  reduction=Reduction.SUM, name='scc')
...
Test with logits.
    ...
    def test_no_reduction(self):
        y_true = K.constant([[0], [1], [2]])
        logits = K.constant([[8., 1., 1.], [0., 9., 1.], [2., 3., 5.]])
        cce_obj = losses.SparseCategoricalCrossentropy(  from_logits=True, reduction=Reduction.NONE)
        loss = cce_obj(y_true, logits)
        assert np.allclose(K.eval(loss), (0.001822, 0.000459, 0.169846), atol=1e-3)

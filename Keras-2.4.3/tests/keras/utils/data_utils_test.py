...
class SlowSequence(Sequence):
    ...
    def __getitem__(self, item):
        if self.wait:  self.wait = False  time.sleep(40)
        return np.ones(self.shape, dtype=np.uint32) * item * self.inner
...
def test_generator_enqueuer_threadsafe():
    ...
    enqueuer.start(3, 10)
    gen_output = enqueuer.get()
    with pytest.raises(RuntimeError) as e:gen_output) for _ in range(10)]
...
def DISABLED_test_on_epoch_end_threads_sequence_change_length():
    ...
    for i in range(50):
        acc.append(next(gen_output)[0, 0, 0, 0])
    assert acc == list([k * 5 for k in range(50)]) 'Order was not keep in GeneratorEnqueuer with processes')
    enqueuer.stop()
...
@pytest.mark.skipif('TRAVIS_PYTHON_VERSION' in os.environ,'Takes 150s to run')
def DISABLED_test_missing_inputs():
    missing_idx = 10

    class TimeOutSequence(DummySequence):
        def __getitem__(self, item):  if item == missing_idx:      time.sleep(120)  return super(TimeOutSequence, self).__getitem__(item)
    ...
    with pytest.warns(UserWarning, match=warning_msg):
        for _ in range(11):  next(gen_output)

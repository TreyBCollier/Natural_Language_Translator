...
def test_has_arg(fn, name, accept_all, expected):
    if isinstance(fn, str):
        context = dict()
        try:  exec('def {}: pass'.format(fn), context)
        except SyntaxError:  if sys.version_info >= (3,):        pytest.skip('Function is not compatible with Python 2')Sometimes exec adds builtins to the context

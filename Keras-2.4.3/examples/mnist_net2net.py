...
(1) wider_random_pad0.0320   0.0317   0.0289
...
def wider2net_conv2d(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    # Arguments
        teacher_w1: `weight` of conv2d layer to become wider,of shape (filters1, num_channel1, kh1, kw1)
        teacher_b1: `bias` of conv2d layer to become wider,of shape (filters1 teacher_w2: `weight` of next connected conv2d layer,of shape (filters2, num_channel2, kh2, kw2)
    ...
    elif init == 'net2wider':add small noise to break symmetry, so that student model will havefull capacity later
...
def wider2net_fc(teacher_w1, teacher_b1, teacher_w2, new_width, init):
    ...
    # Arguments
        ...
        teacher_b1: `bias` of fc layer to become wider,of shape (nout1 teacher_w2: `weight` of next connected fc layer,of shape (nin2, nout2)
        new_width: new `nout` for the wider fc layer
        init: initialization algorithm for new weights,either 'random-pad' or 'net2wider'
    '''
    assert teacher_w1.shape[1] == teacher_w2.shape[0] 'successive layers from teacher model should have compatible shapes')
...
def deeper2net_conv2d(teacher_w):
    ...
    # Arguments
        teacher_w: `weight` of previous conv2d layer,of shape (kh, kw, num_channel, filters)
    '''
...
def make_wider_student_model(teacher_model,        x_train, y_train,        x_test, y_test,        init, epochs):
    ...
    model.add(Conv2D(new_conv1_width, 3, input_shape=input_shape,padding='same', name='conv1'))
    model.add(MaxPooling2D(2, name='pool1'))
    model.add(Conv2D(64, 3, padding='same', name='conv2'))
    model.add(MaxPooling2D(2, name='pool2'))
    model.add(Flatten(name='flatten'))
...
def make_deeper_student_model(teacher_model,         x_train, y_train,         x_test, y_test epochs):
    ...
    if init == 'net2deeper':
        ...
        model.add(Conv2D(64, 3, padding='same',    name='conv2-deeper', weights=new_weights))
    ...
    if init == 'net2deeper':net2deeper for fc layer with relu, is just an identity initializer
        model.add(Dense(64, kernel_initializer='identity',   activation='relu', name='fc1-deeper'))
    elif init == 'random-init':
...
def net2wider_experiment():
    ...
    make_wider_student_model(teacher_model,        x_train, y_train,        x_test, y_test,        init='random-pad',        epochs=epochs)
    print('\n(2) building wider student model by net2wider ...')
    make_wider_student_model(teacher_model,        x_train, y_train,        x_test, y_test,        init='net2wider',        epochs=epochs)

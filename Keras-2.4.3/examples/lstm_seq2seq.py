...
- In inference mode, when we want to decode unknown input sequences, we:
    ...
    - Start with a target sequence of size 1the start-of-sequence character)
    - Feed the state vectors and 1-char target sequence
        to the decoder to produce predictions for the next character
    - Sample the next character using these predictionssimply use argmax).
...
for i, (input_text, target_text) in enumerate(zip(input_texts, target_texts)):
    ...
    for t, char in enumerate(target_text):decoder_target_data is ahead of decoder_input_data by one timestep
...
Exit condition: either hit max lengthor find stop character.

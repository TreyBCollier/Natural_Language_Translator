...
--style-imageimage to learn style from
--style-mask semantic labels for style image
--target-masksemantic labels for target image (your doodle)
...
parser.add_argument('--nlabels', type=int,'number of semantic labels' (regions in differnet colors)' in style_mask/target_mask')
parser.add_argument('--style-image', type=str,'path to image to learn style from')
parser.add_argument('--style-mask', type=str,'path to semantic mask of style image')
parser.add_argument('--target-mask', type=str,'path to semantic mask of target image')
parser.add_argument('--content-image', type=str, default=None,'path to optional content image')
parser.add_argument('--target-image-prefix', type=str,'path prefix for generated results')
...
def load_mask_labels():
    ...
    target_mask = np.stack([target_mask_label == r for r in range(num_labels)],      axis=stack_axis)
...
def style_loss(style_image, target_image, style_masks, target_masks):
    ...
    for i in range(num_labels):
        if K.image_data_format() == 'channels_first':  style_mask = style_masks[i, :, :]  target_mask = target_masks[i, :, :]
        else:  style_mask = style_masks[:, :, i]  target_mask = target_masks[:, :, i]

...
void pcolor(CoordinateArray &x,  CoordinateArray &y,  ColorArray &d,  unsigned int rows,  unsigned int cols,  float bounds[4],  int interpolation,  OutputArray &out)
{
    if (rows >= 32768 || cols >= 32768 throw std::runtime_error("rows and cols must both be less than 32768");
    }
    ...
    if (rows == 0 || cols == 0 throw std::runtime_error("Cannot scale to zero size");
    }

    if (d.dim(2) != 4 throw std::runtime_error("data must be in RGBA format");
    }

    // Check dimensions match
    unsigned long nx = x.dim(0);
    unsigned long ny = y.dim(0);
    if (nx != (unsigned long)d.dim(1) || ny != (unsigned long)d.dim(0) throw std::runtime_error("data and axis dimensions do not match");
    }
 ...
 Copy data to output buffer
        for (i = 0; i < rows; i++     for (j = 0; j < cols; j++         alpha = arows[i];      beta = acols[j];
      a00 = alpha * beta;      a01 = alpha * (1.0 - beta);      a10 = (1.0 - alpha) * beta;      a11 = 1.0 - a00 - a01 - a10;
      for (size_t k = 0; k < 4; ++k  position[k      d(rowstart[i], colstart[j], k) * a00 +   d(rowstart[i], colstart[j] + 1, k) * a01 +   d(rowstart[i] + 1, colstart[j], k) * a10 +   d(rowstart[i] + 1, colstart[j] + 1, k) * a11;             position += 4; }
    }
}

template <class CoordinateArray, class ColorArray, class Color, class OutputArray>
void pcolor2(CoordinateArray &x,   CoordinateArray &y,   ColorArray &d,   unsigned int rows,   unsigned int cols,   float bounds[4],   Color &bg,   OutputArray &out)
{
    ...
    // Check we have something to output to
    if (rows == 0 || cols == 0 throw std::runtime_error("rows or cols is zero; there are no pixels");
    }
    ...
    unsigned long nx = x.dim(0);
    unsigned long ny = y.dim(0);
    if (nx != (unsigned long)d.dim(1) + 1 || ny != (unsigned long)d.dim(0) + 1 throw std::runtime_error("data and axis bin boundary dimensions are incompatible");
    }

    if (bg.dim(0) != 4 throw std::runtime_error("bg must be in RGBA format");
    }
    ...
    for (i = 0; i < rows; i++ for (j = 0; j < cols; j++     if (irows[i] == -1 || jcols[j] == -1         memcpy(position, (const unsigned char *)bg.data(), 4 * sizeof(unsigned char));  } else {      for (size_t k = 0; k < 4; ++k  position[k] = d(irows[i], jcols[j], k);            position += 4;    }
}

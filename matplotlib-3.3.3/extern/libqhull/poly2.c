...
/*-<a       href="qh-poly.htm#TOC"
  ...
  notes:
    ...
    if notverified>0 at end of routine
      ...
      a lens-shaped component, these points were not verified'Qi Tv' to verify all points.  (Exhaustive check also verifies)
...
below %2.2g of the nearest %sfacet.\n",   qh_setsize(facets), maxoutside, (qh ONLYgood ?  "good " : ""));
  FOREACHfacet_i_(facets) {  /* for each point with facet assignment */
    ...
    if (dist > maxoutside(qh ONLYgood && !bestfacet->good&& !((bestfacet= qh_findgooddist(point, bestfacet, &dist, &facetlist))     && dist > maxoutside))
        notgood++;
      else {
        waserror= True;
        qh_fprintf(qh ferr, 6109, "qhull precision error: point p%d is outside facet f%d, distance= %6.8g maxoutside= %6.8g\n",      facet_i, bestfacet->id, dist, maxoutside);
        if (errfacet1 != bestfacet   errfacet2= errfacet1;errfacet1= bestfacet;      }
...
void qh_check_dupridge(facetT *facet1, realT dist1, facetT *facet2, realT dist2) {
  ...
  FOREACHvertex_(facet1->vertices) {     /* The duplicate ridge is between facet1 and facet2, so either facet can be tested */
    FOREACHvertexA_(facet1->vertices(vertex > vertexA){   /* Test each pair once */
...
/*-<a       href="qh-poly.htm#TOC"
  ...
  design:
    ...
    for each point with an assigned facet
      find the best facet for the point and check all coplanar facetsouter planes)
...
void qh_check_maxout(void) {
  ...
  && (qh PRINTsummary || qh KEEPinside || qh KEEPcoplanar qh TRACElevel || qh PRINTstatistics qh PRINTout[0] == qh_PRINTsummary || qh PRINTout[0] == qh_PRINTnone)) {
    trace1((qh ferr, 1023, "qh_check_maxout: determine actual maxoutside and minvertex\n"));
    vertices= qh_pointvertex(/*qh.facet_list*/);
    FORALLvertices {
      FOREACHneighbor_(vertex zinc_(Zdistvertex);  /* distance also computed by main loop below */
        qh_distplane(vertex->point, neighbor, &dist);
        minimize_(minvertex, dist);
        if (-dist > qh TRACEdist || dist > qh TRACEdist neighbor == qh tracefacet || vertex == qh tracevertex)qh_fprintf(qh ferr, 8093, "qh_check_maxout: p%d(v%d) is %.2g from f%d\n",          qh_pointid(vertex->point), vertex->id, dist, neighbor->id);
      }
    }
    if (qh MERGING) {
  ...
  do {
    ...
    FOREACHfacet_i_(facets) {     /* for each point with facet assignment */
      if (facet point= qh_point(facet_i);
        if (point == qh GOODpointp)continue;
        zzinc_(Ztotcheck);
        qh_distplane(point, facet, &dist);
        numpart++;
        bestfacet= qh_findbesthorizon(qh_IScheckmax, point, facet, !qh_NOupper, &dist, &numpart);
        if (bestfacet && dist > maxoutside   if (qh ONLYgood && !bestfacet->good&& !((bestfacet= qh_findgooddist(point, bestfacet, &dist, &facetlist))     && dist > maxoutside))  notgood++;else  maxoutside= dist;        if (dist > qh TRACEdist || (bestfacet && bestfacet == qh tracefacet))qh_fprintf(qh ferr, 8094, "qh_check_maxout: p%d is %.2g above f%d\n",qh_pointid(point), dist, (bestfacet ? bestfacet->id : UINT_MAX));
      }
    }
  }while
    (maxoutside > 2*old_maxoutside);
    /* if qh.maxoutside increases substantially, qh_SEARCHdist is not valide.g., RBOX 5000 s Z1 G1e-13 t1001200614 | qhull */
...
void qh_check_point(pointT *point, facetT *facet, realT *maxoutside, realT *maxdist, facetT **errfacet1, facetT **errfacet2) {
  ...
  qh_distplane(point, facet, &dist);
  if (dist > *maxoutside) {
    if (*errfacet1 != faceterrfacet2= *errfacet1;
      *errfacet1= facet;
    }
...
/*-<a       href="qh-poly.htm#TOC"
 ...
 all facets
        for all pointscheck that point is inside facet
*/
...
of a facet.\n");
    ...
    if (qh PRINTprecision(testouter)
...
all %sfacets.  Will make %2.0f distance computations.\n",    maxoutside, (qh ONLYgood ?  "good " : ""), total);
    ...
    FORALLfacets {
      ...
      if (facet->flipped)
        continue;
      if (!facet->normal qh_fprintf(qh ferr, 7061, "qhull warning (qh_check_points): missing normal for facet f%d\n", facet->id);
        continue;
    (testouter) {
#if qh_MAXoutside
        maxoutside= facet->maxoutside + 2* qh DISTround; one DISTround to actual point and another to computed point */
#endif
    {
        if (point != qh GOODpointp)qh_check_point(point, facet, &maxoutside, &maxdist, &errfacet1, &errfacet2);
      }
      FOREACHpoint_(qh other_points if (point != qh GOODpointp)qh_check_point(point, facet, &maxoutside, &maxdist, &errfacet1, &errfacet2);
      }
    }
    if (maxdist > qh outside_err) {
...
/*-<a       href="qh-poly.htm#TOC"
  ...
  qh_checkconvex( facetlist, fault )
    ...
    fault = qh_DATAfault if reporting errors= qh_ALGORITHMfault otherwise
  ...
  note:
    if not merging,
      ...
      tests vertices for neighboring simplicial   facets
 centrums of neighboring facets
...
void qh_checkconvex(facetT *facetlist, int fault) {
  ...
  FORALLfacet_(facetlist) {
    ...
    if (qh MERGING && (!qh ZEROcentrum || !facet->simplicial || facet->tricoplanar))
      ...
      allsimplicial= True;
      neighbor_i= 0;
      FOREACHneighbor_(facet vertex= SETelemt_(facet->vertices, neighbor_i++, vertexT);
        if (!neighbor->simplicial || neighbor->tricoplanar   allsimplicial= False;continue;        qh_distplane(vertex->point, neighbor, &dist);
        if (dist > -qh DISTround   if (fault == qh_DATAfault     qh_precision("coplanar or concave ridge");  qh_fprintf(qh ferr, 6114, "qhull precision error: initial simplex is not convex. Distance=%.2g\n", dist);  qh_errexit(qh_ERRsingular, NULL, NULL);}if (dist > qh DISTround     zzinc_(Zconcaveridges);  qh_precision("concave ridge");  qh_fprintf(qh ferr, 6115, "qhull precision error: f%d is concave to f%d, since p%d(v%d) is %6.4g above\n",    facet->id, neighbor->id, qh_pointid(vertex->point), vertex->id, dist);  errfacet1= facet;  errfacet2= neighbor;  waserror= True;}else if (qh ZEROcentrum     if (dist > 0) {     /* qh_checkzero checks that dist < - qh DISTround */    zzinc_(Zcoplanarridges);    qh_precision("coplanar ridge");    qh_fprintf(qh ferr, 6116, "qhull precision error: f%d is clearly not convex to f%d, since p%d(v%d) is %6.4g above\n",      facet->id, neighbor->id, qh_pointid(vertex->point), vertex->id, dist);    errfacet1= facet;    errfacet2= neighbor;    waserror= True;   }else {  zzinc_(Zcoplanarridges);  qh_precision("coplanar ridge");  trace0((qh ferr, 22, "qhull precision error: f%d may be coplanar to f%d, since p%d(v%d) is within %6.4g during p%d\n",    facet->id, neighbor->id, qh_pointid(vertex->point), vertex->id, dist, qh furthest_id));}      }
    }
    if (!allsimplicial(qh CENTERtype == qh_AScentrum if (!facet->center)facet->center= qh_getcentrum(facet);
        centrum= facet->center;
      }else {
        if (!centrum_warning && (!facet->simplicial || facet->tricoplanar)    centrum_warning= True; qh_fprintf(qh ferr, 7062, "qhull warning: recomputing centrums for convexity test.  This may lead to false, precision errors.\n");        centrum= qh_getcentrum(facet);
        tempcentrum= True;
      }
      FOREACHneighbor_(facet if (qh ZEROcentrum && facet->simplicial && neighbor->simplicial)continue;
        if (facet->tricoplanar || neighbor->tricoplanar)continue;
        zzinc_(Zdistconvex);
        qh_distplane(centrum, neighbor, &dist);
        if (dist > qh DISTround   zzinc_(Zconcaveridges);qh_precision("concave ridge");qh_fprintf(qh ferr, 6117, "qhull precision error: f%d is concave to f%d.  Centrum of f%d is %6.4g above f%d\n",  facet->id, neighbor->id, facet->id, dist, neighbor->id);errfacet1= facet;errfacet2= neighbor;waserror= True;if (dist >= 0.0) {   /* if arithmetic always rounds the same,     can test against centrum radius instead */zzinc_(Zcoplanarridges);qh_precision("coplanar ridge");qh_fprintf(qh ferr, 6118, "qhull precision error: f%d is coplanar or concave to f%d.  Centrum of f%d is %6.4g above f%d\n",  facet->id, neighbor->id, facet->id, dist, neighbor->id);errfacet1= facet;errfacet2= neighbor;waserror= True;    (tempcentrum)
...
void qh_checkfacet(facetT *facet, boolT newmerge, boolT *waserrorp) {
  ...
  if (facet->simplicial) {
    ...
    ||(qh hull_dim == 2 && numridges + numvertices + numneighbors != 6)(!facet->degenerate && !facet->redundant qh_fprintf(qh ferr, 6125, "qhull internal error (qh_checkfacet): for facet f%d, #ridges %d < #neighbors %d or(3-d) > #vertices %d or(2-d) not all 2\n",  facet->id, numridges, numneighbors, numvertices);
        waserror= True;
      }
    }
  }
  ...
  if (!facet->simplicial) {
    FOREACHneighbor_(facet(!neighbor->seen qh_fprintf(qh ferr, 6132, "qhull internal error (qh_checkfacet): facet f%d does not have a ridge for neighbor f%d\n",    facet->id, neighbor->id);
        errother= neighbor;
        waserror= True;
     qh_vertexintersect_new(facet->vertices, neighbor->vertices);
      ...
      FOREACHvertex_(intersection)
        vertex->seen= True;
      FOREACHridge_(facet->ridges if (neighbor != otherfacet_(ridge, facet))  continue;
        FOREACHvertex_(ridge->vertices   if (!vertex->seen     qh_fprintf(qh ferr, 6133, "qhull internal error (qh_checkfacet): vertex v%d in r%d not in f%d intersect f%d\n",        vertex->id, ridge->id, facet->id, neighbor->id);  qh_errexit(qh_ERRqhull, facet, ridge);}vertex->seen2= True;    (!newmerge FOREACHvertex_(intersection   if (!vertex->seen2     if (qh IStracing >=3 || !qh MERGING       qh_fprintf(qh ferr, 6134, "qhull precision error (qh_checkfacet): vertex v%d in f%d intersect f%d but\n\
 not in a ridge.  This is ok under merging.  Last point was p%d\n",vertex->id, facet->id, neighbor->id, qh furthest_id);    if (!qh FORCEoutput && !qh MERGING         qh_errprint("ERRONEOUS", facet, neighbor, NULL, vertex);      if (!qh MERGING)        qh_errexit(qh_ERRqhull, NULL, NULL);       }
      }
      qh_settempfree(&intersection);
    }
  }else { /* simplicial */
    FOREACHneighbor_(facet(neighbor->simplicial skipA= SETindex_(facet->neighbors, neighbor);
        skipB= qh_setindex(neighbor->neighbors, facet);
        if (skipA<0 || skipB<0 || !qh_setequal_skip(facet->vertices, skipA, neighbor->vertices, skipB)   qh_fprintf(qh ferr, 6135, "qhull internal error (qh_checkfacet): facet f%d skip %d and neighbor f%d skip %d do not match \n",         facet->id, skipA, neighbor->id, skipB);errother= neighbor;waserror= True;      }
    }
  }
  if (qh hull_dim < 5 && (qh IStracing > 2 || qh CHECKfrequently)) {
    FOREACHridge_i_(facet->ridges   /* expensive */
      for (i=ridge_i+1; i < ridge_n; i++ ridge2= SETelemt_(facet->ridges, i, ridgeT);
        if (qh_setequal(ridge->vertices, ridge2->vertices)   qh_fprintf(qh ferr, 6227, "Qhull internal error (qh_checkfacet): ridges r%d and r%d have the same vertices\n",        ridge->id, ridge2->id);errridge= ridge;waserror= True;      }
    }
  }
  if (waserror) {
...
void qh_checkflipped_all(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    if (facet->normal && !qh_checkflipped(facet, &dist, !qh_ALL)) {
      ...
      if (!qh FORCEoutput qh_errprint("ERRONEOUS", facet, NULL, NULL, NULL);
        waserror= True;
      }
    }
  }
...
void qh_checkpolygon(facetT *facetlist) {
  ...
  FORALLfacet_(facetlist) {
    if (facet == qh visible_list)
      visibleseen= True;
    if (!facet->visible(!nextseen if (facet == qh facet_next)nextseen= True;
        else if (qh_setsize(facet->outsideset)   if (!qh NARROWhull
#if !qh_COMPUTEfurthest     || facet->furthestdist >= qh MINoutside
#endif        qh_fprintf(qh ferr, 6137, "qhull internal error (qh_checkpolygon): f%d has outside points before qh facet_next\n",facet->id);  qh_errexit(qh_ERRqhull, facet, NULL);}    +;
 ...
 += qh_setsize(facet->ridges);
    FOREACHvertex_(facet->vertices>visitid++;
      if (!vertex->seen vertex->seen= True;
        numvertices++;
        if (qh_pointid(vertex->point) == qh_IDunknown   qh_fprintf(qh ferr, 6139, "qhull internal error (qh_checkpolygon): unknown point %p for vertex v%d first_point %p\n",         vertex->point, vertex->id, qh first_point);waserror= True;      }
    }
  }
  ...
  if (facetlist == qh facet_list) {
    ...
    if (qh VERTEXneighbors{
      ...
      if (totvneighbors != totvertices qh_fprintf(qh ferr, 6141, "qhull internal error (qh_checkpolygon): vertex neighbors inconsistent.  Totvneighbors %d, totvertices %d\n",      totvneighbors, totvertices);
        waserror= True;
      }
    }
...
void qh_checkvertex(vertexT *vertex) {
  ...
  if (!waserror && !vertex->deleted) {
    if (qh_setsize(vertex->neighbors)) {
      FOREACHneighbor_(vertex if (!qh_setin(neighbor->vertices, vertex)   qh_fprintf(qh ferr, 6146, "qhull internal error (qh_checkvertex): neighbor f%d does not contain v%d\n", neighbor->id, vertex->id);errfacet= neighbor;waserror= True;      }
    }
  }
  if (waserror) {
    qh_errprint("ERRONEOUS", NULL, NULL, NULL, vertex);
...
void qh_clearcenters(qh_CENTER type) {
  ...
  if (qh CENTERtype != type) {
    FORALLfacets {
      if (facet->tricoplanar && !facet->keepcentrum)facet->center= NULL;  /* center is owned by the ->keepcentrum facet */
      else if (qh CENTERtype == qh_ASvoronoi){
        if (facet->center   qh_memfree(facet->center, qh center_size);facet->center= NULL;      }else /* qh.CENTERtype == qh_AScentrum * if (facet->center   qh_memfree(facet->center, qh normal_size);facet->center= NULL;      }
    }
    qh CENTERtype= type;
  }
...
void qh_createsimplex(setT *vertices) {
  ...
  FORALLnew_facets {
    nth= 0;
    FORALLfacet_(qh newfacet_list(facet != newfacet)
...
facetT *qh_findbestfacet(pointT *point, boolT bestoutside, realT *bestdist, boolT *isoutside) {
  ...
  bestfacet= qh_findbest(point, qh facet_list,       bestoutside, !qh_ISnewfacets, bestoutside /* qh_NOupper */,       bestdist, isoutside, &totpart);
  if (*bestdist < -qh DISTround) {
    bestfacet= qh_findfacet_all(point, bestdist, isoutside, &numpart);
    totpart += numpart;
    if ((isoutside && *isoutside && bestoutside)
    || (isoutside && !*isoutside && bestfacet->upperdelaunay) qh_findbest(point, bestfacet,       bestoutside, False, bestoutside,       bestdist, isoutside, &totpart);
      totpart += numpart;
    }
  }
...
facetT *qh_findbestlower(facetT *upperfacet, pointT *point, realT *bestdistp, int *numpart) {
  ...
  if (!bestfacet) {
    ...
    qh_vertexneighbors();
    FOREACHneighbor_(vertex(neighbor->upperdelaunay || neighbor->flipped)
        continue;
      (*numpart)++;
      qh_distplane(point, neighbor, &dist);
      if (dist > bestdist bestfacet= neighbor;
        bestdist= dist;
      }
    }
  }
...
facetT *qh_findfacet_all(pointT *point, realT *bestdist, boolT *isoutside,     int *numpart) {
  ...
  *isoutside= False;
  FORALLfacets {
    ...
    totpart++;
    qh_distplane(point, facet, &dist);
    if (dist > *bestdistbestdist= dist;
      bestfacet= facet;
      if (dist > qh MINoutside *isoutside= True;
        break;
      }
    }
  }
...
int qh_findgood(facetT *facetlist, int goodhorizon) {
  ...
  if (qh GOODvertex>0 && !qh MERGING) {
    FORALLfacet_(facetlist(!qh_isvertex(qh GOODvertexp, facet->vertices) facet->good= False;
        numgood--;
      }
    }
  }
  if (qh GOODpoint && numgood) {
    FORALLfacet_(facetlist(facet->good && facet->normal zinc_(Zdistgood);
        qh_distplane(qh GOODpointp, facet, &dist);
        if ((qh GOODpoint > 0) ^ (dist > 0.0)   facet->good= False;numgood--;      }
    }
  }
  if (qh GOODthreshold && (numgood || goodhorizon || qh GOODclosest)) {
    FORALLfacet_(facetlist(facet->good && facet->normal if (!qh_inthresholds(facet->normal, &angle)   facet->good= False;numgood--;if (angle < bestangle     bestangle= angle;  bestfacet= facet;}      }
    }
    if (!numgood && (!goodhorizon || qh GOODclosest)(qh GOODclosest if (qh GOODclosest->visible)qh GOODclosest= NULL;
        else {qh_inthresholds(qh GOODclosest->normal, &angle);if (angle < bestangle)  bestfacet= qh GOODclosest;    (bestfacet && bestfacet != qh GOODclosest if (qh GOODclosest)qh GOODclosest->good= False;
...
void qh_findgood_all(facetT *facetlist) {
  ...
  if (qh GOODvertex <0 || (qh GOODvertex > 0 && qh MERGING)) {
    FORALLfacet_(facetlist(facet->good && ((qh GOODvertex > 0) ^ !!qh_isvertex(qh GOODvertexp, facet->vertices)) if (!--numgood   if (qh ONLYgood     qh_fprintf(qh ferr, 7064, "qhull warning: good vertex p%d does not match last good facet f%d.  Ignored.\n",     qh_pointid(qh GOODvertexp), facet->id);  return;}else if (qh GOODvertex > 0)  qh_fprintf(qh ferr, 7065, "qhull warning: point p%d is not a vertex('QV%d').\n",      qh GOODvertex-1, qh GOODvertex-1);else  qh_fprintf(qh ferr, 7066, "qhull warning: point p%d is a vertex for every facet('QV-%d').\n",      -qh GOODvertex - 1, -qh GOODvertex - 1);        facet->good= False;
      }
    }
  }
  startgood= numgood;
  if (qh SPLITthresholds) {
    FORALLfacet_(facetlist(facet->good if (!qh_inthresholds(facet->normal, &angle)   facet->good= False;numgood--;if (angle < bestangle     bestangle= angle;  bestfacet= facet;}      }
    }
    if (!numgood && bestfacet>good= True;
      numgood++;
...
#endif
      if (dist > bestdist bestfacet= facet;
        bestdist= dist;
      }
    }
  }
...
void qh_initbuild( void) {
  ...
  if (qh GOODvertex > 0)
    ...
    qh GOODvertexp= qh_point(-qh GOODvertex-1);
  if ((qh GOODpoint
       && (qh GOODpointp < qh first_point  /* also catches !GOODpointp */ || qh GOODpointp > qh_point(qh num_points-1)))
    || (qh GOODvertex (qh GOODvertexp < qh first_point  /* also catches !GOODvertexp */  || qh GOODvertexp > qh_point(qh num_points-1)))) {
    qh_fprintf(qh ferr, 6150, "qhull input error: either QGn or QVn point is > p%d\n",   qh num_points-1);
    qh_errexit(qh_ERRinput, NULL, NULL);
  }
  maxpoints= qh_maxmin(qh first_point, qh num_points, qh hull_dim);
  if (qh SCALElast)
    qh_scalelast(qh first_point, qh num_points, qh hull_dim,     qh MINlastcoord, qh MAXlastcoord, qh MAXwidth);
  qh_detroundoff();
  if (qh DELAUNAY && qh upper_threshold[qh hull_dim-1] > REALmax/2        && qh lower_threshold[qh hull_dim-1] < -REALmax/2) {
    for (i=qh_PRINTEND; i--; (qh PRINTout[i] == qh_PRINTgeom && qh DROPdim < 0&& !qh GOODthreshold && !qh SPLITthresholds)
        break;  /* in this case, don't set upper_threshold */
    }
    if (i < 0(qh UPPERdelaunay) { /* matches qh.upperdelaunay in qh_setfacetplane */
        qh lower_threshold[qh hull_dim-1]= qh ANGLEround * qh_ZEROdelaunay;
        qh GOODthreshold= True;
      }else {
        qh upper_threshold[qh hull_dim-1]= -qh ANGLEround * qh_ZEROdelaunay;
        if (!qh GOODthreshold)qh SPLITthresholds= True; /* build upper-convex hull even if Qg *//* qh_initqhull_globals errors if Qg without Pdk/etc. */
      }
    }
  }
  ...
  if (qh PRINToptions1st || qh TRACElevel || qh IStracing) {
    if (qh TRACElevel || qh IStracing)
      qh_fprintf(qh ferr, 8103, "\nTrace level %d for %s | %s\n",IStracing ? qh IStracing : qh TRACElevel, qh rbox_command, qh qhull_command);
  ...
  if (qh ONLYgood) {
    ...
    if (!(qh GOODthreshold || qh GOODpoint| (!qh MERGEexact && !qh PREmerge && qh GOODvertexp))) {
      qh_fprintf(qh ferr, 6152, "qhull input error: 'Qg' (ONLYgood) needs a good threshold('Pd0D0'), a\n\
good point(QGn or QG-n), or a good vertex with 'QJ' or 'Q0' (QVn).\n");
      qh_errexit(qh_ERRinput, NULL, NULL);
    }
    if (qh GOODvertex > 0  && !qh MERGING  /* matches qh_partitionall */ !qh_isvertex(qh GOODvertexp, vertices) qh_findbestnew(qh GOODvertexp, qh facet_list,     &dist, !qh_ALL, &isoutside, &numpart);
      zadd_(Zdistgood, numpart);
      if (!isoutside qh_fprintf(qh ferr, 6153, "qhull input error: point for QV%d is inside initial simplex.  It can not be made a vertex.\n",     qh_pointid(qh GOODvertexp));
        qh_errexit(qh_ERRinput, NULL, NULL);
    (!qh_addpoint(qh GOODvertexp, facet, False) qh_settempfree(&vertices);
 ...
 >toporient ^= (unsigned char)True;
  ...
  FORALLfacets {
    if (!qh_checkflipped(facet, NULL, !qh_ALL)) {  /* can happen with 'R0.1' */
      if (qh DELAUNAY && ! qh ATinfinity if (qh UPPERdelaunay)qh_fprintf(qh ferr, 6240, "Qhull precision error: Initial simplex is cocircular or cospherical.  Option 'Qs' searches all points.  Can not compute the upper Delaunay triangulation or upper Voronoi diagram of cocircular/cospherical points.\n");
    ...
    FOREACHneighbor_(facet qh_getangle(facet->normal, neighbor->normal);
...
setT *qh_initialvertices(int dim, setT *maxpoints, pointT *points, int numpoints) {
  ...
  if (qh ALLpoints)
    ...
    while (qh_setsize(simplex) != dim+1 qh_RANDOMint;
      randr= randr/(qh_RANDOMmax+1);
      idx= (int)floor(qh num_points * randr);
      while (qh_setin(simplex, qh_point(idx)) idx++; /* in case qh_RANDOMint always returns the same value */
    ...
    FOREACHpoint_i_(maxpoints(point_i & 0x1) {     /* first pick up max. coord. points */
        if (!qh_setin(simplex, point) && !qh_setin(tested, point)){qh_detsimplex(point, simplex, k, &nearzero);if (nearzero)  qh_setappend(&tested, point);else {  qh_setappend(&simplex, point);  if (++k == dim)  /* use search for last point */    break;}      }
    }
    while (k != dim && (point= (pointT*)qh_setdellast(maxpoints))(!qh_setin(simplex, point) && !qh_setin(tested, point)){
        qh_detsimplex(point, simplex, k, &nearzero);
        if (nearzero)qh_setappend(&tested, point);
        else {qh_setappend(&simplex, point);k++;      }
    }
    idx= 0;
    while (k != dim && (point= qh_point(idx++))(!qh_setin(simplex, point) && !qh_setin(tested, point)){
        qh_detsimplex(point, simplex, k, &nearzero);
        if (!nearzero){qh_setappend(&simplex, point);k++;      }
    }
  ...
  design:
    ...
    repeat twice -- once to make best matches, once to match the rest
      for each possible facet in qh.hash_table
        if it is a matching facet and pass 2make matchunless tricoplanar, mark match for merging (qh_MERGEridge)[e.g., tricoplanar RBOX s 1000 t993602376 | QHULL C-1e-3 d Qbb FA Qt]
        if it is a matching facet and pass 1test if this is a better match
      if pass 1,
...
void qh_matchduplicates(facetT *atfacet, int atskip, int hashsize, int *hashcount) {
  ...
  hash= qh_gethash(hashsize, atfacet->vertices, qh hull_dim, 1,SETelem_(atfacet->vertices, atskip));
...
void qh_nearcoplanar(void /* qh.facet_list */) {
  ...
  if (!qh KEEPcoplanar && !qh KEEPinside) {
    ...
    numpart= 0;
    FORALLfacets {
      if (facet->coplanarset FOREACHpoint_(facet->coplanarset   numpart++;qh_distplane(point, facet, &dist);if (dist < innerplane     if (!qh KEEPinside)    SETref_(point)= NULL;}else if (!qh KEEPcoplanar)  SETref_(point)= NULL;        qh_setcompact(facet->coplanarset);
      }
    }
    zzadd_(Zcheckpart, numpart);
  }
...
vertexT *qh_nearvertex(facetT *facet, pointT *point, realT *bestdistp) {
  ...
  if (facet->tricoplanar) {
    ...
    center= facet->center;
    FOREACHneighbor_(apex(neighbor->center == center FOREACHvertex_(neighbor->vertices)qh_setappend(&vertices, vertex);
      }
    }
  }else
...
int qh_newhashtable(int newsize) {
  ...
  while (True) {
    if (newsize<0 || size<0 qh_fprintf(qhmem.ferr, 6236, "qhull error (qh_newhashtable): negative request (%d) or size (%d).  Did int overflow due to high-D?\n", newsize, size); /* WARN64 */
...
ridgeT *qh_nextridge3d(ridgeT *atridge, facetT *facet, vertexT **vertexp) {
  ...
  FOREACHridge_(facet->ridges) {
    if (ridge == atridge)
      continue;
    if ((ridge->top == facet) ^ qh_ORIENTclock SETsecondt_(ridge->vertices, vertexT);
    ...
    if (vertex == atvertex(vertexp) othervertex;
      return ridge;
    }
  }
  return NULL;
...
void qh_outcoplanar(void /* facet_list */) {
  ...
  FORALLfacets {
    FOREACHpoint_(facet->outsidesetnum_outside--;
      if (qh KEEPcoplanar || qh KEEPnearinside qh_distplane(point, facet, &dist);
        zinc_(Zpartition);
...
setT *qh_pointfacet(void /*qh.facet_list*/) {
  ...
  qh vertex_visit++;
  FORALLfacets {
    FOREACHvertex_(facet->vertices(vertex->visitid != qh vertex_visit vertex->visitid= qh vertex_visit;
...
/*-<a       href="qh-poly.htm#TOC"
  ...
  design:
    for each hash entry
      if defined
        if unmatched or will merge (NULL, qh_MERGEridge, qh_DUPLICATEridge)print entry and neighbors
*/
void qh_printhashtable(FILE *fp) {
  ...
  FOREACHfacet_i_(qh hash_table) {
    if (facet) {
      FOREACHneighbor_i_(facet if (!neighbor || neighbor == qh_MERGEridge || neighbor == qh_DUPLICATEridge)break;
    (neighbor_i == neighbor_n)
        continue;
      qh_fprintf(fp, 9283, "hash %d f%d ", facet_i, facet->id);
      FOREACHvertex_(facet->vertices)
        qh_fprintf(fp, 9284, "v%d ", vertex->id);
      qh_fprintf(fp, 9285, "\n neighbors:");
      FOREACHneighbor_i_(facet if (neighbor == qh_MERGEridge)id= -3;
        else if (neighbor == qh_DUPLICATEridge)id= -2;
        id= getid_(neighbor);
...
void qh_setvoronoi_all(void) {
  ...
  FORALLfacets {
    if (!facet->normal || !facet->upperdelaunay || qh UPPERdelaunay(!facet->center)
...
void qh_triangulate(void /*qh.facet_list*/) {
  ...
  for (facet= new_facet_list; facet && facet->next; facet= nextfacet) { /* null facets moved to end */
    ...
    if (facet->visible)
      continue;
    if (facet->ridges(qh_setsize(facet->ridges) > 0 qh_fprintf(qh ferr, 6161, "qhull error (qh_triangulate): ridges still defined for f%d\n", facet->id);
        qh_errexit(qh_ERRqhull, facet, NULL);
      }
  ...
  FORALLfacet_(new_facet_list) {
    if (facet->tricoplanar && !facet->visible) {
      FOREACHneighbor_i_(facet if (neighbor_i == 0) {  /* first iteration */if (neighbor->tricoplanar)  orig_neighbor= neighbor->f.triowner;else  orig_neighbor= neighbor;{if (neighbor->tricoplanar)  otherfacet= neighbor->f.triowner;else  otherfacet= neighbor;if (orig_neighbor == otherfacet     zinc_(Ztridegen);  facet->degenerate= True;  break;}      }
    }
  }
  ...
  for (facet= new_facet_list; facet && facet->next; facet= nextfacet) { /* may delete facet */
    nextfacet= facet->next;
    if (facet->visible(facet->tricoplanar) { /* a null or mirrored facet */
        qh_delfacet(facet);
        qh num_visible--;
      }else {  /* a non-simplicial facet followed by its tricoplanars */
        if (visible && !owner   /*  RBOX 200 s D5 t1001471447 | QHULL Qt C-0.01 Qx Qc Tv Qt -- f4483 had 6 vertices/neighbors and 8 ridges */trace2((qh ferr, 2053, "qh_triangulate: all tricoplanar facets degenerate for non-simplicial facet f%d\n",  visible->id));qh_delfacet(visible);qh num_visible--;        visible= facet;
        owner= NULL;
      }
    }else if (facet->tricoplanar(facet->f.triowner != visible || visible==NULL qh_fprintf(qh ferr, 6162, "qhull error (qh_triangulate): tricoplanar facet f%d not owned by its visible, non-simplicial facet f%d\n", facet->id, getid_(visible));
        qh_errexit2(qh_ERRqhull, facet, visible);
    (owner)
        facet->f.triowner= owner;
      else if (!facet->degenerate owner= facet;
        ...
        if (!qh TRInormals) { /* center and normal copied to tricoplanar facets */visible->center= NULL;visible->normal= NULL;        qh_delfacet(visible);
        qh num_visible--;
      }
    }
  }
...
/*-<a       href="qh-poly.htm#TOC"
  ...
  design:
      if qh_ASvoronoi,Voronoi center (facet->center)
...
void qh_triangulate_facet(facetT *facetA, vertexT **first_vertex) {
  ...
  if (qh CENTERtype == qh_ASvoronoi && !facetA->center  /* matches upperdelaunay in qh_setfacetplane() */ fabs_(facetA->normal[qh hull_dim -1]) >= qh ANGLEround * qh_ZEROdelaunay) {
